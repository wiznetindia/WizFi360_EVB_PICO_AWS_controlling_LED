...
/* Added by Teju */
uint8_t mqtt_sub_msg[MQTT_SUB_MAX_SIZE] = {
    0,
};
/**
 * ----------------------------------------------------------------------------------------------------
...
void mqtt_event_callback(MQTTContext_t *pContext, MQTTPacketInfo_t *pPacketInfo, MQTTDeserializedInfo_t *pDeserializedInfo)
{
    ...
    if ((pPacketInfo->type & 0xF0U) == MQTT_PACKET_TYPE_PUBLISH)
    {
        ...
        {
            sprintf(mqtt_sub_msg,"%.*s",pDeserializedInfo->pPublishInfo->payloadLength, pDeserializedInfo->pPublishInfo->pPayload);
            printf("AWS IOT message is %s\n",mqtt_sub_msg);
           // printf("AWs Iot Message is %.*s,%d,%.*s\n", pDeserializedInfo->pPublishInfo->topicNameLength, pDeserializedInfo->pPublishInfo->pTopicName,
